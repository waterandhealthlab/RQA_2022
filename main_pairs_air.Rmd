---
title: "RQA analysis personal air temperature"
...
# read all csv files that contain "A". A means skin data
temperature_measurements <- dir_ls("data/temperature data/", regexp = "A") %>% 
...
# file name as code
names(temperature_measurements) %<>% str_match("T\\d\\d_A\\d") %>% {glue("{str_sub(.,1,3)}-{str_sub(.,6,6)}")}
...
cat(glue("<ul><b>Participants:</b>
          ...
          <li>We drop 1 participant because of invalid biomarker measurements ({length(keep)} participants left)</li></ul>"))
